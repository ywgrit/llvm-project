...
// These are internal relocation numbers for local got relaxation. They aren't part
// of the psABI spec.
...
// Returns true if the pair of relocations at i and i + 2 can be relaxed.
...
static void relaxPcalaAddi(const InputSection &sec, size_t i, uint64_t loc,
  ...
  const uint64_t symval =
      (r_hi.expr == R_LOONGARCH_PLT_PAGE_PC ? r_hi.sym->getPltVA() : r_hi.sym->getVA()) + r_hi.addend;
  ...
  // remove the first insn: pcalau12i
